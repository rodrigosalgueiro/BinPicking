...
main (int argc, char *argv[])
{
  ...
  if (pcl::io::loadPCDFile (scene_filename_, *scene) < 0)
  {
	...
	for(size_t i=0; i<model_filenames_.size(); ++i)
{

  if (pcl::io::loadPCDFile (model_filenames_[i], *model) < 0)
  {
...
{
 std::cout << "loading model cloud " << model_filenames_[i] << std::endl;
}
...
/*pcl::VoxelGrid<PointType> sor;
  sor.setInputCloud (boost::make_shared<pcl::PointCloud<pcl::PointXYZ> >(*model));
  sor.setLeafSize (res_scene, res_scene, res_scene);
  sor.filter (*model);

 res_model = computeCloudResolution(model);
std::cout << "Modelfiltered resolution:  " << res_model << std::endl;*/
